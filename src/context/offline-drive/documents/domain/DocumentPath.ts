...
export class DocumentPath extends Path {
  ...
  hasSameDirname(other: DocumentPath): boolean {
  ...
  hasSameExtension(other: DocumentPath): boolean {
    return this.extension() === other.extension();
  }

  changeFolder(folder: string): DocumentPath {
    return DocumentPath.fromParts([folder, this.nameWithExtension()]);
  }

  updateName(name: string): DocumentPath {
    return DocumentPath.fromParts([this.dirname(), name]);
  }
}
