...
export class FolderRepositorySynchronizer {
  ...
  async run(remoteFolders: Array<Folder>, allRemoteFolderIds: Set<number>): Promise<void> {
    ...
    const foldersToDelete = currentFolders.filter((folder: Folder) => {
      return !folder.isRoot() && !remoteFoldersIds.has(folder.id) && allRemoteFolderIds.has(folder.id);
    });
