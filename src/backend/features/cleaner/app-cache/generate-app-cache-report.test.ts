...
jest.mock('@internxt/drive-desktop-core/build/backend', () => ({
  ...
  CleanerModule: {
    ...
    scanSubDirectory: jest.fn(),
  },
}));

describe('generateAppCacheReport', () => {
  const mockedScanDirectory = jest.mocked(CleanerModule.scanDirectory);
  const mockedGetAppCachePaths = jest.mocked(getAppCachePaths);
  const mockedScanSubDirectory = jest.mocked(CleanerModule.scanSubDirectory);
