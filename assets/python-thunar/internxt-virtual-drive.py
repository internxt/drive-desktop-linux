...
class InternxtVirtualDrive(GObject.GObject, Thunarx.MenuProvider):
    ...
    def __init__(self):
        ...
        # Every activation gets an id and only the newest may write the
        # clipboard. Two activations inside REQUEST_TIMEOUT_SECONDS otherwise
        # race, the slower response lands last, and the user pastes the link
        # for the file they did not ask for. Both the increment and the
        # comparison run on the GTK main context (the activate handler and the
        # idle callback), so the counter needs no lock; a worker thread only
        # carries the value it captured.
    ...
    def _copy_to_clipboard_once(self, value, request_id):
        """The `idle_add` callback, on the main context.

        Drops the result of a superseded request. A worker started earlier can
        finish later, and without this check its stale link would overwrite the
        one the user asked for most recently.
    ...
    def _request_link(self, url, request_id):
        """Ask the local API for a sharing link. Runs on a worker thread.

        `request_id` identifies the activation this worker serves; it is handed
        back to the idle callback, which discards it if a later activation has
        superseded it.
        """
        ...
        if link:
            ...
            GLib.idle_add(self._copy_to_clipboard_once, link, request_id)
    ...
    def _copy_internxt_link(self, menu_item, file_info):
        ...
        worker = threading.Thread(
            target=self._request_link,
            args=(url, self._latest_request_id),
            daemon=True)
        worker.start()
